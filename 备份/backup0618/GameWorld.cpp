...
{
  for (auto i : worldobj)
    if (i->can_bump() && i->getstate())
      if (i->is_enermy())
        if (!i->is_bullet())
          if (check_bump(i, b_bu))
          {
            //i->set_HP_A(i->get_HP_A() - b_bu->get_effect_b());
            i->set_HP_A(i->get_HP_A() - (5 + 3 * powerup));
      ...
      if (!i->is_enermy())
        if (i->is_bullet())
          if (check_bump(i, obj))
          {
            ...
            {
              //obj->set_HP_A(obj->get_HP_A() - (i->get_effect_b()));
              obj->set_HP_A(obj->get_HP_A() - (5 + 3 * powerup));
...
bool GameWorld::Goodie_bump(Goodie *obj)
{
  if (check_bump(obj, me))
    return true;
  else
    return false;
}
...
// set Blue Bullet
void GameWorld::set_bullet(int mx, int my, double size, double effect)
{
  worldobj.emplace_back(
    new Blue_Bullet(mx, my + 50, size, this, effect));
}
...
void GameWorld::add_HP() 
{ 
  me->set_HP(me->get_HP() + 50); 
  if (me->get_HP() > 100)
    me->set_HP(100);
}

void GameWorld::set_HP_Goodie(int x, int y)
{
  worldobj.emplace_back(new HP_Goodie(x, y, this));
}

void GameWorld::set_Meteor_Goodie(int x, int y)
{
  worldobj.emplace_back(new Meteor_Goodie(x, y, this));
}

void GameWorld::set_Powerup_Goodie(int x, int y)
{
  worldobj.emplace_back(new Powerup(x, y, this));
}

bool GameWorld::Meteor_bump(int x, int y)
{
  for (auto i : worldobj)
    if (i->getstate() && i->can_bump() && i->is_enermy()
      && !(i->is_bullet()))
      i->set_HP_A(0);
  return true;
}

void GameWorld::set_meteor(int mx, int my)
{
  worldobj.emplace_back(new Meteor(mx, my + 100, this));
}
...
/********************
You can not invoke these 4 functions.
*************************/
void GameWorld::Init()
{
...
LevelStatus GameWorld::Update()
{
  ...
  if (des_num >= 3 * GetLevel() && me->getstate())
    return LevelStatus::LEVEL_CLEARED;

  for (auto i = worldobj.begin(); i != worldobj.end();) // delete dead object
  {
    if (!(*i)->getstate())
    {
...
// needn`t to set level!
  std::string mes("HP: " + std::to_string(me->get_HP()) + 
    ...
    "   Enemies: " + std::to_string(des_num) + "/" + std::to_string(required) + 
