...
//  ObjectBase(imageID, x, y, direction, layer, size), state(true),
//  findworld(world), HP_A(HP_A) { }
//GameObject::GameObject(int imageID, int x, int y, int direction,
//  int layer, double size, int effect_b, GameWorld *world) :
//  ObjectBase(imageID, x, y, direction, layer, size), state(true), findworld(world),
//  HP_A(0), effect_b(effect_b) {}



int GameObject::get_HP_A() const { return HP_A; }
void GameObject::set_HP_A(int n_HP_A) { HP_A = n_HP_A; }
int GameObject::get_effect_b() const { return effect_b; }
void GameObject::set_effect_b(int e) { effect_b = e; }
...
Dawnbreaker::Dawnbreaker(GameWorld* g)
    : GameObject(IMGID_DAWNBREAKER, 300, 100, 0, 0, 1.0, g), energy(10), HP(2147483647) {}
...
Projectile::Projectile(int imageID, int x, int y, int direction, int layer,
                       double size, GameWorld* world, int effect)
