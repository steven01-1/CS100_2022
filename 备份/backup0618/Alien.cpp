...
Alien::Alien(int imageID, int x, int y, int direction, int layer,
	...
	GameObject(imageID, x, y, direction, layer, size, world),
	energy(energy), effect(effect), strategy(strategy),
...
void Alien::cause_damage(int d) { set_HP_A(get_HP_A() - d); }
...
void Sigma::show_death()
{
	...
	findworld->add_des_num();
	if (randInt(1, 100) <= 20) // drop a goodie 
	{
